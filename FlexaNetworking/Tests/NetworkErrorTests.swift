    ...
    override class func spec() {
        describe("isUnauthorized") {
            context("status is 401") {
                let subject = NetworkError.invalidStatus(status: 401, resource: DefaultTestAPIResource(), request: nil)
            ...
            context("status is not 401") {
                ...
                let subject = NetworkError.invalidStatus(status: status, resource: DefaultTestAPIResource(), request: nil)
                it("is false") {
