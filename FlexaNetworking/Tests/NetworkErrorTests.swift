    ...
    override class func spec() {
        describe("isUnauthorized") {
            context("status is 401") {
                let subject = NetworkError.invalidStatus(status: 401, resource: DefaultTestAPIResource())
            ...
            context("status is not 401") {
                ...
                let subject = NetworkError.invalidStatus(status: status, resource: DefaultTestAPIResource())
                it("is false") {
